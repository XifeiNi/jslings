...
import { MinifyError, CodeEngineInterface } from '../../types/code-engine.types'
...
export class CodeEngine implements CodeEngineInterface {
    ...
    minifiedCode?: string
    constructor(userdata: UserData, database: Map<string, Exercise>) {
    ...
    async minifyCodeAndCheckSyntax() : Promise<void> {
        try {
            const localCode = obtainStringifiedFileContents(this.userdata.current.info.path)
    ...
    watchForChanges() {
        ...
        const args: string[] = [this.userdata.current.info.testPath, '--watch']
        try {
            run(args)
        } catch(err) {
            throw err
